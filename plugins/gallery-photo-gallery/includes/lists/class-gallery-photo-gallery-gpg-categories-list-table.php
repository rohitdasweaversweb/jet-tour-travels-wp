<?php
ob_start();
class Gallery_Categories_List_Table extends WP_List_Table{
    private $plugin_name;
    ...
    public function __construct($plugin_name) {
        ...
        add_action( 'admin_notices', array( $this, 'gallery_category_notices' ) );
    }
     ...
     * @return mixed
     */
    public static function get_gallery_categories( $per_page = 20, $page_number = 1, $search = '' ) {
    ...
    public function get_gpg_gallery_category( $id ) {
        global $wpdb;

        $sql = "SELECT * FROM {$wpdb->prefix}ays_gpg_gallery_categories WHERE id=" . absint( sanitize_text_field( $id ) );
    ...
    public function add_edit_gpg_gallery_category( $data ){
        global $wpdb;
                ...
                if($ays_change_type != ''){
                    if($id == null){
                        $url = esc_url_raw( add_query_arg( array(
                            "action"    => "edit",
                            "gpg_gallery_category"  => $wpdb->insert_id,
                            "status"    => $message
                        ) ) );
                    }else{
                    ...
                    $url = esc_url_raw( remove_query_arg(array('action', 'gpg_gallery_category')  ) ) . '&status=' . $message;
                    wp_redirect( $url );
                }
            }
        }
    ...
    public static function delete_gallery_categories( $id ) {
        global $wpdb;
        $wpdb->delete(
            "{$wpdb->prefix}ays_gpg_gallery_categories",
            array( 'id' => $id ),
            array( '%d' )
        );
    }
    ...
    public function no_items() {
        echo __( 'There are no gallery categories yet.', $this->plugin_name );
    }
    ...
    function column_title( $item ) {
        ...
        $actions = array(
            'edit' => sprintf( '<a href="?page=%s&action=%s&gpg_gallery_category=%d">'. __('Edit', $this->plugin_name) .'</a>', esc_attr( $_REQUEST['page'] ), 'edit', absint( $item['id'] ) ),
        );
        
        // if(intval($item['id']) !== 1){
            $actions['delete'] = sprintf( '<a class="ays_confirm_del" data-message="%s" href="?page=%s&action=%s&gpg_gallery_category=%s&_wpnonce=%s">'. __('Delete', $this->plugin_name) .'</a>', $restitle, esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['id'] ), $delete_nonce );
        // }
    ...
    function column_items_count( $item ) {
        ...
        if( isset( $item['id'] ) && absint( sanitize_text_field( $item['id'] ) ) > 0){
            $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}ays_gallery WHERE gpg_categories_id = " . absint( sanitize_text_field($item['id'] ) );
    ...
    public function process_bulk_action() {
        ...
        if ( 'delete' === $this->current_action() ) {
            ...
            if ( ! wp_verify_nonce( $nonce, $this->plugin_name . '-delete-gpg-gallery-category' ) ) {
                die( 'Go get a life script kiddies' );
            }
            else {
                self::delete_gallery_categories( absint( $_GET['gpg_gallery_category'] ) );

                // esc_url_raw() is used to prevent converting ampersand in url to "#038;"
                // add_query_arg() return the current url

                $url = esc_url_raw( remove_query_arg(array('action', 'gpg_gallery_category', '_wpnonce')  ) ) . '&status=deleted';
                wp_redirect( $url );
            }
        ...
        if ( ( isset( $_POST['action'] ) && $_POST['action'] == 'bulk-delete' ) || ( isset( $_POST['action2'] ) && $_POST['action2'] == 'bulk-delete' ) ) {
            ...
            foreach ( $delete_ids as $id ) {
                self::delete_gallery_categories( $id );
    ...
    public function gallery_category_notices(){
        ...
        if ( 'created' == $status )
            $updated_message = esc_html( __( 'Gallery category created.', $this->plugin_name ) );
        elseif ( 'updated' == $status )
            $updated_message = esc_html( __( 'Gallery category saved.', $this->plugin_name ) );
        elseif ( 'deleted' == $status )
            $updated_message = esc_html( __( 'Gallery category deleted.', $this->plugin_name ) );
